...
namespace fre {
	...
	double EuroCall::d_plus(double S0, double sigma, double r)
	{
		return (log(S0 / K) + (r + 0.5 * pow(sigma, 2.0)) * T) / (sigma * sqrt(T));
	}
	double EuroCall::d_minus(double S0, double sigma, double r)
	{
		return d_plus(S0, sigma, r) - sigma * sqrt(T);
	}
	double EuroCall::PriceByBSFormula(double S0, double sigma, double r)
	{
		return S0 * N(d_plus(S0, sigma, r)) - K * exp(-r * T) * N(d_minus(S0, sigma, r));
	}
	double EuroCall::VegaByFormula(double S0, double sigma, double r)
	{
		double pi = 4.0 * atan(1.0);
